...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Errorf("get host error : ", err)
		}
		port, err := netCmd.Flags().GetInt16("port")
		if err != nil {
			fmt.Errorf("get port error : ", port)
		}

		fmt.Println("disconnecting to ", host, " : ", port)
	},
}

func init() {
	actionCmd.AddCommand(disconnectCmd)
